  ...
  const Game: React.FC = () => {
    ...
    const [isSnakeAlive, setIsSnakeAlive] = useState(true);
    ...
    const moveSnake = () => {
      if (!isPaused && isStarted && isSnakeAlive) {
          ...
          {isStarted && isSnakeAlive ? (
            <div className="grid" style={{ gridTemplateColumns: `repeat(${GRID_SIZE}, ${cellSize}px)` }}>
              ...
              return (
                <div
                  key={index}
                  className={`cell w-${cellSize} h-${cellSize} ${
                    isSnakeCell
                      ? 'snake-cell'
                      : isObstacleCell
            ...
            <div className="start-container">
              <p>{isSnakeAlive ? 'Press "Start" to begin the game!' : 'Oops! Snake died. Press "Start" to try again.'}</p>
              <button
                onClick={startGame}
